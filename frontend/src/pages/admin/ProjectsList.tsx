...
export default function ProjectsList() {
  ...
  const del = useMutation({
    mutationFn: (slug: string) => apiDelete(`/api/projects/${slug}`),
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-2xl font-bold">项目管理</h1>
        </div>
        <Link to="/admin/projects/new" className="px-4 py-2 rounded-xl bg-[var(--color-accent)] text-[var(--color-bg)] text-sm font-medium hover:brightness-110 transition-all">
          + 新建项目
        </Link>
      </div>
      ...
      <div className="glass-card overflow-hidden" style={{ borderRadius: 16 }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {projects?.map((p) => (
              <tr key={p.id} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-accent-soft)] transition-colors">
                ...
                <td className="px-5 py-3.5 text-right">
                  <Link to={`/admin/projects/${p.slug}/edit`} className="text-xs text-[var(--color-accent)] hover:underline mr-4">编辑</Link>
            ...
            {!projects?.length && (
              <tr><td colSpan={4} className="px-5 py-8 text-center text-xs text-[var(--color-text-tertiary)]">暂无项目</td></tr>
            )}
          </tbody>
        </table>
      </div>
